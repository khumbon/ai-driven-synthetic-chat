...
export const LegalInsightsEmail = ({ data }: { data: ReportData }) => {
  console.log('lah patterns');
  console.log(data.patterns);
  const patternData = data.patterns.slice(0, 5);

  const timeSavingsCalculation = {
    avgTimePerQuestion: 15,
    totalQuestions: data.summary.totalUserQuestions,
    totalTimeSaved: (data.summary.totalUserQuestions * 15) / 60,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Chip
          label={`📊 ${data.summary.totalConversations} Conversations Analyzed`}
          sx={{
      ...
      <CardContent sx={{ p: 3 }}>
          ...
          <Grid size={{ xs: 6 }}>
            <Card sx={{ p: 2, textAlign: 'center', backgroundColor: '#f8fafc' }}>
              <Typography variant="h4" fontWeight="bold" color="primary">
                {data.summary.totalUserQuestions}
              </Typography>
          ...
          <Grid size={{ xs: 6 }}>
            <Card sx={{ p: 2, textAlign: 'center', backgroundColor: '#f8fafc' }}>
              <Typography variant="h4" fontWeight="bold" color="secondary">
                {data.summary.avgQuestionsPerChat}
              </Typography>
              <Typography variant="caption" color="text.secondary">
                Avg Questions/Chat
              </Typography>
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          <Grid size={{ xs: 6 }}>
            ...
            <Stack spacing={1}>
              {Object.entries(data.privacyTopics).map(([topic, count]) => (
          ...
          <Grid size={{ xs: 6 }}>
            ...
            <Stack spacing={1}>
              {Object.entries(data.commercialContractTopics).map(([topic, count]) => (
                <Box key={topic} sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  <Typography variant="body2">{topic}</Typography>
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 3 }}>
          {data.mostCommonTerms.slice(0, 8).map(([term, count]) => (
            <Chip
              key={term}
              label={`${term} (${count})`}
              size="small"
